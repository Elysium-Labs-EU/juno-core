...
const EmailPosition = ({
  ...
  useEffect(() => {
    if (
      threadsObjectStart.current === undefined ||
      (threadsObjectStart.current &&
        threadsObjectStart.current.threads.length <
          activeEmailList.threads.length)
    ) {
    ...
    <S.Wrapper>
      {threadsObjectStart.current && (
        <GS.TextMutedParagraph>
          {viewIndex + 1} / {threadsObjectStart.current.threads.length}
        </GS.TextMutedParagraph>
      )}
    </S.Wrapper>
  )
